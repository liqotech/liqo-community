# Liqo Maintainers Meeting Minutes

## Meeting Details

- **Date:** 2026-02-04
- **Time:** 17:30 - 18:30 (CEST)

## Participants
- Alessandro Olivero
- Claudio Lorina
- Francesco Cheinasso
- Francesco Torta
- Fulvio Risso

## Agenda

- Open Agenda
- AOB

## Minutes

* New release: decided to go for a new release (1.1), although the number of new features are limited. It would include also the bug fixes merged so far, particulatly the one about dropped packets with Cilium dataplane v2 in Google.
* Review of all the open PR, selected which ones should be merged in 1.1. Mantainers decided also who will be in charge of finalizing each PR. 
* Decided to meet again in one week in order to review the status of the PRs, and finalize the release.
